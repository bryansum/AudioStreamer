...
extension Streamer: DownloadingDelegate {
    ...
    public func download(_: Downloading, changedState downloadState: DownloadingState) {
        os_log("%@ - %d [state: %@]", log: Streamer.logger, type: .debug, #function, #line, String(describing: downloadState))
    }

    public func download(_: Downloading, didReceiveData data: Data, progress: Float) {
